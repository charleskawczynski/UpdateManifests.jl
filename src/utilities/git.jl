const UpdateManifests_GIT_COMMITTER_NAME = "UpdateManifests Julia"
const UpdateManifests_GIT_COMMITTER_EMAIL = "UpdateManifests_noreply@julialang.org"
...
function get_git_name_and_email(; env=ENV)
    name = if haskey(env, "GIT_COMMITTER_NAME")
        ...
        UpdateManifests_GIT_COMMITTER_NAME
    end

    email = if haskey(env, "GIT_COMMITTER_EMAIL")
        env["GIT_COMMITTER_EMAIL"]
    else
        UpdateManifests_GIT_COMMITTER_EMAIL
    end
...
function git_get_main_branch(main_branch::DefaultBranch)
    return string(strip(read(`git rev-parse --abbrev-ref HEAD`, String)))
end
git_get_main_branch(main_branch::AbstractString) = main_branch
