...
function pr_info(
    ...
    new_pr_title = m"""
    $(pr_title_prefix)UpdateManifests: add new compat entry for $(name) at version
...
function pr_info(
    ...
    new_pr_title = m"""
    $(pr_title_prefix)UpdateManifests: bump compat for $(name) to
    $(compat_entry_for_latest_version)$(subdir_string)$(pr_title_parenthetical)
    """
...
function create_new_pull_request(
    ...
    main_branch_name::AbstractString,
...
function make_pr_for_new_version(
    ...
    main_branch::Union{DefaultBranch,AbstractString}=DefaultBranch(),
    ...
    with_temp_dir(; cleanup=true) do ssh_private_key_dir
        ...
        with_temp_dir(; cleanup=true) do tmpdir
            ...
            main_branch_name = git_get_main_branch(main_branch)
            git_checkout(main_branch_name)

            # Create UpdateManifests branch and check it out
            new_branch_name = "UpdateManifests/new_version/$(get_random_string())"
...
function force_ci_trigger(
    ...
    # https://github.com/JuliaRegistries/UpdateManifests.jl/issues/387
