@testset "main.jl" begin
    @testset "unknown ci config" begin
        struct MockCI <: UpdateManifests.CIService end

        @test_throws ErrorException("Unknown CI Config: MockCI") UpdateManifests.main(
            ENV, MockCI()
        )
    end
    ...
    @testset "no pr GitHub" begin
        ...
        cd(tmpdir) do
            ...
            apply(patches) do
                withenv(
                    "GITHUB_REPOSITORY" => "UpdateManifests.jl", "GITHUB_TOKEN" => "token"
                ) do
                    prs = UpdateManifests.main()
                    @test length(prs) == 0
                end
            end
        end
        ...
        mktempdir() do tmpdir
            cd(tmpdir) do
                ...
                apply(patches) do
                    withenv(
                        "GITLAB_CI" => "true",
                        "CI_PROJECT_PATH" => "UpdateManifests.jl",
                        "GITLAB_TOKEN" => "token",
