@testset "get_project_deps" begin
    @testset "no jll" begin
        apply([git_clone_patch, project_toml_patch, cd_patch]) do
            deps = UpdateManifests.get_project_deps(
...
@testset "clone_all_registries" begin
    ...
    apply([mktempdir_patch, git_clone_patch]) do
        UpdateManifests.clone_all_registries([
...
@testset "get_latest_version_from_registries!" begin
    ...
    deps = Set{UpdateManifests.DepInfo}([
        # No version specified
        UpdateManifests.DepInfo(UpdateManifests.Package(packageA, UUID(0))),

        # Version is less than what is in registry
        UpdateManifests.DepInfo(
            UpdateManifests.Package(packageB, UUID(1)); latest_version=VersionNumber(1)
        ),

        # Version is greater than what is in registry
        UpdateManifests.DepInfo(
            UpdateManifests.Package(packageC, UUID(2)); latest_version=VersionNumber("3")
        ),
    ])

    apply([clone_all_registries_patch, rm_patch]) do
        result = UpdateManifests.get_latest_version_from_registries!(
...
@testset "get_existing_registries!" begin
    ...
    result = UpdateManifests.get_existing_registries!(deps, joinpath(@__DIR__, "deps"))
