function _cleanup_old_branches(url)
    ...
    cd(git_repo_dir) do
        ...
        branches = [
            ...
            contains(b, "UpdateManifests/new_version/") || contains(b, "integration/")
        ]
...
function _generate_branch_name(name::AbstractString)
    return "integration/$(UpdateManifests.get_random_string())/$(name)"
end
...
function with_main_branch(
    f::Function,
    path_to_content::AbstractString,
    repo_url::AbstractString,
    parent_branch::AbstractString,
)
    b = generate_branch("main", path_to_content, repo_url, parent_branch)
    return f(b)
end
...
function generate_branch(
    ...
    path_to_content::AbstractString,
    repo_url::AbstractString,
    parent_branch::AbstractString="main",
)
    b = _generate_branch_name(name)
    git_repo_dir = with_cloned_repo(repo_url)

    cd(git_repo_dir) do
        UpdateManifests.git_checkout(parent_branch)
        UpdateManifests.git_branch(b; checkout=true)
        ...
        UpdateManifests.api_retry(() -> UpdateManifests.git_add())
        UpdateManifests.api_retry(
            () -> UpdateManifests.git_commit(
                "Automatic commit - UpdateManifests integration tests"
            ),
        )
        UpdateManifests.api_retry(() -> UpdateManifests.git_push("origin", b))
...
function with_cloned_repo(repo_url)
    git_repo_dir = joinpath(mktempdir(), UpdateManifests.LOCAL_REPO_NAME)
    UpdateManifests.api_retry(() -> UpdateManifests.git_clone(repo_url, git_repo_dir))
