...
git_clone_patch = @patch function UpdateManifests.git_clone(
    url::AbstractString, p::AbstractString
)
    return nothing
end
git_push_patch = @patch function UpdateManifests.git_push(
    ::AbstractString, ::AbstractString, ::Union{AbstractString,Nothing}; kwargs...
)
    return nothing
end
git_gmb_patch = @patch UpdateManifests.git_get_main_branch(main_branch) = nothing
git_checkout_patch = @patch UpdateManifests.git_checkout(branch) = nothing
...
clone_all_registries_patch = @patch function UpdateManifests.clone_all_registries(
...
decode_pkey_patch = @patch function UpdateManifests.decode_ssh_private_key(::AbstractString)
    return "pkey_info"
end

pr_titles_mock = @patch function UpdateManifests.get_pr_titles(
    ::GitForge.Forge, ::GitHub.Repo, ::String
)
    return ["UpdateManifests: bump compat for PackageA to 1, (keep existing compat)", "foo"]
end

function make_clone_https_patch(dir::AbstractString)
    return @patch function UpdateManifests.get_url_with_auth(
...
function make_clone_ssh_patch(dir::AbstractString)
    return @patch function UpdateManifests.get_url_for_ssh(
        ::GitForge.Forge, ::CIService, ::Union{GitHub.Repo,GitLab.Project}
    )
        return dir
    end
...
gh_gpr_patch = @patch function UpdateManifests.get_pull_requests(
...
gl_gpr_patch = @patch function UpdateManifests.get_pull_requests(
...
function get_prs_patch(prs)
    return @patch function UpdateManifests.get_pull_requests(
...
gh_make_pr_patch = @patch function UpdateManifests.make_pr_for_new_version(
    ::GitHub.GitHubAPI, ::GitHub.Repo, ::DepInfo, ::EntryType, ::CIService; kwargs...
)
    return GitHub.PullRequest()
end

gl_make_pr_patch = @patch function UpdateManifests.make_pr_for_new_version(
